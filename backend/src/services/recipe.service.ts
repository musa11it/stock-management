...
export async function listRecipes(query: { page?: number; limit?: number; search?: string }) {
  const { skip, take, page, limit } = resolvePagination(query);
  const where: Prisma.RecipeWhereInput = query.search ? { name: { contains: query.search, mode: 'insensitive' } } : {};
...
export async function createRecipe(
  input: { name: string; description?: string; menuItemId?: string; ingredients: RecipeIngredientInput[] },
  actorId: string,
) {
...
export async function updateRecipe(
  id: string,
  input: { name?: string; description?: string; menuItemId?: string | null; ingredients?: RecipeIngredientInput[] },
  actorId: string,
) {
  ...
  const recipe = await prisma.$transaction(async (tx) => {
    ...
    return tx.recipe.update({
      where: { id },
      data: { name: input.name, description: input.description, menuItemId: input.menuItemId },
      include: recipeInclude,
    });
  });
